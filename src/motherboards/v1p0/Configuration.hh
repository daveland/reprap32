...
#ifndef BOARDS_REPRAP32_V1P0_CONFIGURATION_HH_
#define BOARDS_REPRAP32_V1P0_CONFIGURATION_HH_

// This file details the pin assignments and features of the RepRap Motherboard
// version 1.2 for the ordinary use case.

//#include "AvrPort.hh"
// We use the AVR32 Port Definitons.   No need for Port or Pin classes
//
#include <avr32/io.h>
#include "compiler.h"
...
#define TX_ENABLE_PIN   Pin(AVR32_PIN_PA13) //Pin(PortC,5)
// The pin that connects to the active-low recieve enable line on the RS485 chip.
//#define RX_ENABLE_PIN   0 //Pin(PortC,7)
...
#define BUZZER_PIN Pin(AVR32_PIN_PA04)   //(PortC,6)

// --- Emergency Stop configuration ---
// Define as 1 if the estop is present, 0 if not.
#define HAS_ESTOP 0
// The pin connected to the emergency stop
#define ESTOP_PIN Pin(AVR32_PIN_PA04) ////Pin(PortE,4)
...
#define X_STEP_PIN      Pin(AVR32_PIN_PB00) //Pin(PortA,6)
// The X direction pin (forward on logic high)
#define X_DIR_PIN       Pin(AVR32_PIN_PB01) //Pin(PortA,5)
// The X stepper enable pin (active low)
#define X_ENABLE_PIN    Pin(AVR32_PIN_PA31) //Pin(PortA,4)
// The X minimum endstop pin (active high)
#define X_MIN_PIN      Pin(0) // Pin(AVR32_PIN_PB10) //Pin(PortB,6)
// The X maximum endstop pin (active high)
#define X_MAX_PIN      Pin(0) // Pin(AVR32_PIN_PB11) //Pin(PortB,5)

// The Y stepper step pin (active on rising edge)
#define Y_STEP_PIN      Pin(AVR32_PIN_PB02) //Pin(PortA,3)
// The Y direction pin (forward on logic high)
#define Y_DIR_PIN       Pin(AVR32_PIN_PB03) //Pin(PortA,2)
// The Y stepper enable pin (active low)
#define Y_ENABLE_PIN    Pin(AVR32_PIN_PA31) //Pin(PortA,1)
// The Y minimum endstop pin (active high)
#define Y_MIN_PIN       Pin(0)  //  Pin(AVR32_PIN_PB08) //Pin(PortB,4)
// The Y maximum endstop pin (active high)
#define Y_MAX_PIN       Pin(0) //Pin(AVR32_PIN_PA09) //Pin(PortH,6)

// The Z stepper step pin (active on rising edge)
#define Z_STEP_PIN      Pin(AVR32_PIN_PA11) //Pin(PortA,0)
// The Z direction pin (forward on logic high)
#define Z_DIR_PIN       Pin(AVR32_PIN_PA12) //Pin(PortH,0)
// The Z stepper enable pin (active low)
#define Z_ENABLE_PIN    Pin(AVR32_PIN_PA30) //Pin(PortH,1)
// The Z minimum endstop pin (active high)
#define Z_MIN_PIN      Pin(0) // Pin(AVR32_PIN_PA21) //Pin(PortH,5)
// The Z maximum endstop pin (active high)
#define Z_MAX_PIN      Pin(0) // Pin(AVR32_PIN_PA22) //Pin(PortH,4)

// The A stepper step pin (active on rising edge)
#define A_STEP_PIN      Pin(AVR32_PIN_PB04) //Pin(PortJ,0)
// The A direction pin (forward on logic high)
#define A_DIR_PIN       Pin(AVR32_PIN_PA20) //Pin(PortJ,1)
// The A stepper enable pin (active low)
#define A_ENABLE_PIN    Pin(AVR32_PIN_PA29) //Pin(PortE,5)

// The B stepper step pin (active on rising edge)
#define B_STEP_PIN      Pin(AVR32_PIN_PB06) //Pin(PortG,5)
// The B direction pin (forward on logic high)
#define B_DIR_PIN       Pin(AVR32_PIN_PB07) //Pin(PortE,3)
// The B stepper enable pin (active low)
#define B_ENABLE_PIN    Pin(AVR32_PIN_PA06) //Pin(PortH,3)
...
#define endstops_invert 0
#define axes_invert 0
